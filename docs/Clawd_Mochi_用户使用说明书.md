...
- ESP-IDF 固件工程
- 电脑端 Python 桥接器，默认蓝牙优先，语音切到 WiFi 后才走局域网
...
| 抱一下 | 爱心表情 |
| 跳舞 | 提高活跃度 |
| 自动桥接 | 蓝牙优先，不主动连接保存的 WiFi |
| 使用蓝牙 | AI 联动只走蓝牙 |
| 使用 WiFi | AI 联动只走 WiFi |
...
- 自动桥接：蓝牙优先，不主动连接保存的 WiFi。
- 使用蓝牙：AI 联动只走蓝牙，电脑和 ESP32 不需要在同一个 WiFi。
- 使用 WiFi：AI 联动只走 WiFi，并开始连接保存的 WiFi；电脑和 ESP32 需要在同一个局域网。

用户可以直接对语音模块说“自动桥接”“使用蓝牙”“使用 WiFi”来切换。切换结果会保存，掉电后仍然生效。
...
这是正常现象。热点 `ClaWD-Mochi` 只是配网和备用控制用。AI 联动默认优先走 BLE 蓝牙，不需要电脑连 ESP32 热点；如果要使用网页、画板或 WiFi 通道，先让 ESP32 和电脑连接同一个正常 WiFi，并对桌宠说“使用 WiFi”。
...
5. 如果只想测试 WiFi 通道，先对桌宠说“使用 WiFi”，确认 ESP32 和电脑在同一个 WiFi，然后执行：
