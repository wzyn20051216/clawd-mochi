...
6. 安装后会启动本机常驻桥接器；它只监听当前电脑自己的 `127.0.0.1`，不会写死用户的局域网 IP。
...
安装成功后，正常打开 Codex 或 Claude Code 即可。AI 开始思考、调用工具、完成任务或报错时，桌宠会自动变化表情并触发语音模块播报。桌宠睡眠只是屏幕和表情的假睡，后台桥接器仍然保持连接；一旦有事件触发，桌宠会自动醒来。
